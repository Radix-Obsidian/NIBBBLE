...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState('ai') // AI profile as primary
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('bucket')) {
          throw new Error('Avatar storage not configured. Using recipe-videos bucket as fallback.')
        }
      ...
      // Fallback to recipe-videos bucket if avatars bucket doesn't exist
      if (error.message.includes('bucket') || error.message.includes('not found')) {
        try {
          ...
          throw new Error(`Avatar upload failed: ${fallbackError}`)
        }
      }
      throw error
    }
  ...
  return (
    <div className='space-y-6'>
      <div>
        <h2 className='text-responsive-xl font-bold text-gray-900'>My Profile</h2>
        <p className='text-responsive text-gray-600 mt-1'>Manage your AI cooking preferences and personal information</p>
      </div>

      {/* Tab Navigation - AI Profile First */}
      ...
      {activeTab === 'ai' && (
        <>
          {/* AI Profile Header */}
          <Card className='p-4 sm:p-6 border border-purple-200 rounded-2xl bg-gradient-to-r from-purple-50 to-indigo-50'>
            <div className='flex items-center space-x-4'>
              <div className='w-16 h-16 rounded-full bg-purple-100 flex items-center justify-center text-2xl'>
                🤖
              </div>
              <div>
                <h3 className='text-xl font-bold text-gray-900'>AI-Powered Cooking Experience</h3>
                <p className='text-gray-600'>Tell us about your cooking style so we can personalize your NIBBBLE experience</p>
              </div>
            </div>
          </Card>
          ...
          <div className='flex justify-end'>
            <button 
              ...
              disabled={saving} 
              className='btn inline-flex items-center justify-center rounded-full bg-purple-600 text-white h-10 px-6 text-sm disabled:opacity-60 w-full sm:w-auto hover:bg-purple-700 transition-colors'
            >
        ...
        <>
          <Card className='p-4 sm:p-6 border border-gray-200 rounded-2xl'>
            ...
            <div className='flex flex-col sm:flex-row sm:items-center gap-4'>
              <div className='w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-semibold flex-shrink-0 overflow-hidden'>
                {form.avatar_url ? (
                  <img 
                    src={form.avatar_url} 
                    alt='Profile'
              ...
              <div className='flex flex-col sm:flex-row gap-2'>
                ...
                <label
                  ...
                  className={`btn rounded-full border border-gray-300 px-4 h-9 text-sm flex items-center justify-center w-full sm:w-auto cursor-pointer ${
                ...
                {form.avatar_url && (
                  <button
                    ...
                    className='btn rounded-full border border-red-300 text-red-600 px-4 h-9 text-sm flex items-center justify-center w-full sm:w-auto hover:bg-red-50'
                  >
                    Remove Photo
                  </button>
                )}
