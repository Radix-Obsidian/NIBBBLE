...
export interface VideoRecipeGridProps {
  recipes: VideoRecipe[];
  title?: string;
  subtitle?: string;
  showViewAll?: boolean;
...
export function VideoRecipeGrid({
  recipes,
  title,
  subtitle,
  showViewAll = false,
  onViewAll
}: VideoRecipeGridProps) {
  return (
    <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6 lg:p-8 shadow-soft border border-white/20">
      ...
      {recipes.length > 0 ? (
        ...
        <div className="text-center py-8 sm:py-12">
          ...
          <p className="text-responsive text-gray-500 mb-4">No video recipes found</p>
          <p className="text-sm text-gray-400">Start creating your first video recipe to see it here!</p>
        </div>
      )}
