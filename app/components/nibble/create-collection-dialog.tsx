// File: app/components/nibble/create-collection-dialog.tsx

'use client';

import React, { useState } from 'react';
import { X, Plus, Minus } from 'lucide-react';
import { CreateCollectionInput, MOOD_TAGS, CUISINE_TYPES, DIETARY_TAGS } from '@/types/nibble-collections';

interface CreateCollectionDialogProps {
  open: boolean;
  onClose: () => void;
  onCreate: (collection: CreateCollectionInput) => void;
}
...
export function CreateCollectionDialog({ open, onClose, onCreate }: CreateCollectionDialogProps) {
  ...
  const toggleDietary = (dietary: string) => {
    setFormData(prev => {
      const prevTags = prev.dietary_tags || [];
      const isSelected = prevTags.includes(dietary);
      return {
        ...prev,
        dietary_tags: isSelected
          ? prevTags.filter(tag => tag !== dietary)
          : [...prevTags, dietary],
      };
    });
  };
